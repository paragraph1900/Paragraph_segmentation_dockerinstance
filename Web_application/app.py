...
# Extracting Most Used font and size combo in each page
                ...
                for page_number in range(number_of_pages):
                    ...
                    for index, string in enumerate(lists):
                        if(string[4].strip()!= ''):
                            if (string[4].find(prev_str)) != -1 :
                     
                               dupl.append(prev_str2)
                        prev_str = string[4].strip()
                        prev_str2 = string[4]
...
#removed minmax for few columns as values are going to e power values and effectig autoencoder
                ...
                cols = df_features_enc.select_dtypes(exclude=['float']).columns

                df_features_enc[cols] = df_features_enc[cols].apply(pd.to_numeric, downcast='float', errors='coerce')
                ...
                clustered = kmeans(cluster_label,df_final,text)
                
                segmented = segmentation(clustered)

                return render_template("download.html", content = segmented)
